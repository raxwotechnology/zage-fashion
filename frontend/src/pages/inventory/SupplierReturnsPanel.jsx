...
const SupplierReturnsPanel = ({ storeId, products }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Supplier return saved (stock deducted)');
      setSupplierId('');
      setReason('damaged');
      setReturnedAt('');
      setNotes('');
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-dark-navy mb-4">Supplier Returns (Damaged / Expired)</h2>
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-card-border p-6 shadow-sm space-y-4">
            ...
            {items.map((l, idx) => (
              <div key={idx} className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                ...
                <div className="sm:col-span-3">
                  <label className="text-xs text-muted-text block mb-1">Unit Cost At Return</label>
                  <input type="number" min="0" step="0.01" value={l.unitCostAtReturn} onChange={(e) => updateLine(idx, { unitCostAtReturn: e.target.value })} className="w-full border border-card-border rounded-xl py-2.5 px-4 text-sm" />
                </div>
        ...
        <div className="flex justify-end">
          <button type="submit" disabled={saving} className="bg-primary-green hover:bg-emerald-600 text-white font-semibold px-6 py-2.5 rounded-xl disabled:opacity-50">
            {saving ? 'Saving...' : 'Save Supplier Return'}
          </button>
        </div>
      </form>
      ...
      <div className="mt-8 bg-white rounded-2xl border border-card-border p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <h3 className="font-semibold text-dark-navy">Recent Supplier Returns</h3>
            ...
            <tbody className="divide-y divide-card-border">
              {history.slice(0, 20).map((r) => (
                <tr key={r._id}>
                  ...
                  <td className="px-4 py-2.5 text-muted-text">{String(r.reason || '').replaceAll('_', ' ')}</td>
                  <td className="px-4 py-2.5 text-muted-text">{r.items?.length || 0}</td>
                </tr>
              ))}
              {history.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-muted-text">No supplier returns found</td>
                </tr>
              )}
            </tbody>
          </table>

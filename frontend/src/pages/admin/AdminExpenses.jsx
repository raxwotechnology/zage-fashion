...
import { Plus, Edit2, Trash2, X, Search, FileDown, Filter } from 'lucide-react';
...
const emptyForm = { title: '', category: 'Utilities', amount: '', date: new Date().toISOString().split('T')[0], status: 'Pending', notes: '' };
      ...
      <div>
          ...
          <div className="flex gap-2">
            <div className="relative group">
              ...
              <div className="hidden group-hover:block absolute right-0 mt-1 bg-white rounded-xl shadow-xl border border-card-border z-20 min-w-[140px]">
                <button onClick={() => exportToCSV(filtered, exportColumns, 'expenses')} className="w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 rounded-t-xl">📄 CSV</button>
                <button onClick={() => exportToExcel(filtered, exportColumns, 'expenses')} className="w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50">📊 Excel</button>
                <button onClick={() => exportToPDF(filtered, exportColumns, 'Expense Report')} className="w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 rounded-b-xl">📋 PDF</button>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-card-border shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-card-border">
                {filtered.map((exp) => (
                  <tr key={exp._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-3.5">
                      <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-purple-100 text-purple-700">{exp.category}</span>
                    </td>
                    <td className="px-6 py-3.5 font-semibold text-dark-navy">Rs. {exp.amount?.toLocaleString()}</td>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-12 text-muted-text text-sm">No expenses found</div>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-card-border flex items-center justify-between">
              <h2 className="text-lg font-bold text-dark-navy">{editingId ? 'Edit Expense' : 'Add Expense'}</h2>
